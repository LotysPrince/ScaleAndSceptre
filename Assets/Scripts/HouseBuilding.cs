...
public class HouseBuilding : MonoBehaviour
{
    ...
    public GameObject houseMiniPrefab;
        ...
        if (Input.GetMouseButtonDown(0) && tileHoveringOn != null)
        {
            if (collisionCount == 0 /*&& spawnCircle.enabled == true*/ && !isPlaced && tileHoveringOn.GetComponent<tileManager>().buildingContained == null)
            {
                ...
                else if (zoningMode)
                {
                    if (zonedTilesNum <= 10)
                    {
                        tileHoveringOn.GetComponent<tileManager>().yellowHighlighted = true;
                        zonedTilesNum += 1;
                        zonedTiles.Add(tileHoveringOn);
                    }
                   
                }
            }
        }
        if (Input.GetMouseButtonDown(1) && zoningMode)
        {
            foreach(var zonedTile in zonedTiles)
            {
